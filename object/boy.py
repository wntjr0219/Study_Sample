...
class Boy:
    ...
    def set_parent_brick(self, brick):
        self.parent_brick = brick
        # 소년의 초기 위치를 발판의 특정 위치로
        self.x, self.y = brick.x + brick.BOY_X0, brick.y + brick.BOY_Y0

    # def act_boy_monster(self):
    #     self.timer = 100
    #     if self.timer == 100:
    #         self.life -= 1
    #         self.timer -= 1

    #     elif self.timer > 0 and self.timer < 100:
    #         self.timer -= 1
    ...
    def update(self):
        ...
        # #발판이 움직이면 소년도 움직임
        if self.parent_brick : #소년이 발판에 붙어있으면
            self.x += self.parent_brick.speed * game_framework.frame_time
            self.x = clamp(self.parent_brick.x - 90, self.x , self.parent_brick.x + 90)

        # 소년이 발판체크
        # for brick in server.bricks:
        #     if collision.collide(self, brick):
        #         self.set_parent_brick(brick)
        #         break # 나머지는 충돌체크할 필요 없음, 중요함
        for monster in server.monsters:
            self.timer = 100
            if self.timer == 100:
                if collision.collide(self, monster):
                    self.life -= 1
                    self.timer -= 1
                break
    ...
    def draw(self):
        ...
        self.font.draw(self.x - 40, self.y + 50, '(Life: %d)' % self.life, (255, 255, 0))
